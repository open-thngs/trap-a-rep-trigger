...
_I2C_ADDRESS = const(41)

class VL53L4CD:
    """Driver for the VL53L4CD distance sensor."""

    def __init__(self, i2c, low_power_mode=False):
        self._i2c = i2c
        time.sleep(0.1)
        self._sensor_init(low_power_mode)

    def _sensor_init(self, is_low_power = True):
        init_seq = (
        ...
        self._write_register(0x0B, b"\x00", addrsize=8)
    ...
    def start_ranging(self):
        ...
        for _ in range(1000):
            ...
            time.sleep(0.001)
        if timed_out:
            raise RuntimeError("Time out waiting for data ready.")
    ...
    def _write_register(self, address, data, addrsize=16):
        self._i2c.writeto_mem(_I2C_ADDRESS, address, data, addrsize=addrsize)
            
    def _read_register(self, address, length=1, addrsize=16):
        return self._i2c.readfrom_mem(_I2C_ADDRESS, address, length, addrsize=addrsize)
